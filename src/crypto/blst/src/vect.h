...
/* eth.zig vendored patch: hoist the __BLST_NO_ASM__ branch above the
 * x86_64/aarch64 64-bit-limb branch. blst's pure-C portable backend (no_asm.h)
 * only defines `llimb_t` for 32-bit limbs, so a no-assembly build must use
 * 32-bit limbs on every architecture, including aarch64/x86_64. Building with
 * -D__BLST_NO_ASM__ then yields the portable C path everywhere. The original
 * upstream ordering checked __x86_64__/__aarch64__ first (forcing 64-bit limbs,
 * which require the assembly backend). The hunk is inert when __BLST_NO_ASM__
 * is not defined. See src/crypto/c-kzg/VENDOR.md. */
