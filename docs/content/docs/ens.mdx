...
ENS names can have associated text records (email, URL, avatar, etc.):

```zig
const eth = @import("eth");

// The resolver module also supports text record lookups
// using the text(bytes32,string) function selector
```
