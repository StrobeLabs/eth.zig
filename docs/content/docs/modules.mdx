...
High-performance cryptographic primitives.
...
| `kzg` | `blobToKzgCommitment`, `computeBlobKzgProof`, `verifyBlobKzgProof`, `computeKzgProof`, `verifyKzgProof`, `computeCellsAndKzgProofs`, `recoverCellsAndKzgProofs`, `verifyCellKzgProofBatch` | EIP-4844 blob and point-evaluation KZG plus EIP-7594 cells (vendored c-kzg-4844 + blst) |
...
| Module | Key Exports | Description |
|--------|-------------|-------------|
| `transaction` | `Transaction`, `LegacyTx`, `Eip1559Tx`, `Eip4844Tx`, `wrapBlobTransaction` | Legacy, EIP-2930, EIP-1559, EIP-4844 transaction types and the blob network wrapper |
| `receipt` | `TransactionReceipt` | Transaction receipt with logs and status |
| `block` | `Block`, `BlockHeader` | Block and block header types |
| `blob` | `Blob`, `BlobSidecar`, `BlobSidecarV1`, `buildSidecarV1`, `NetworkSidecar` | EIP-4844 blobs and EIP-7594 (Fusaka) cell-proof sidecars |
